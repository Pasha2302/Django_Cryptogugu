...
var setCoinTableScroll = () => {
  document.querySelectorAll('.coin-table').forEach(item => {
    ...
    if (item.getElementsByTagName('table')[0].offsetWidth - 10 >= item.offsetWidth) {
...
var setLanguage = (dropdownManager) => {
  var dropdownConfigs = [
    {
  ...
  dropdownConfigs.forEach(({ triggerSelector, targetSelector, className }) => {
    dropdownManager.toggleClassOnClick(triggerSelector, targetSelector, className);
